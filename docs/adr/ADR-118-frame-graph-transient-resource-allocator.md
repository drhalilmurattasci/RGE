...
- **Phase 6 umbrella exit gate.** The 60 fps simple-scene golden gate at `IMPLEMENTATION.md:468` becomes the chapter's exit criterion at the substrate level once the umbrella-composition smoke (dispatch 123 — `crates/gfx/tests/frame_graph_umbrella_smoke.rs`) lands; runtime-perf re-validation against Gate A's recorder-host CLOSED marker (the pre-allocator triangle path certified on NVIDIA GeForce RTX 4060 Ti / Vulkan; commit `35e5078`) is deferred until real pass-record sites grow transient-resource consumers (`FrameRecorder` is currently triangle-only and bypassed by `editor-shell::render_frame`; the just-shipped substrate is NOT what Gate A certified). The frame-graph allocator + ADR-117 handoff together make the future runtime-perf re-run measurable.
