...
- 60fps on `simple-scene` golden project (1k cubes, 1 directional light) **[CLOSED 2026-05-11 on recorder host AND pre-depth gfx path only: NVIDIA GeForce RTX 4060 Ti / Vulkan / DiscreteGpu / 1280×720 / static camera / shared PSO + 1 material / option-(a) single `draw_indexed` / `LitMeshPipeline::new(...)` / `record_lit_mesh_pass(.., None)`; min-of-3 P95 = 0.112 ms (~150× under 16.67 ms gate); NOT universal, NOT vendor parity, NOT cold-start, NOT thermal, NOT CI; see BASELINE.md §6.3]** **[POST-DEPTH PRODUCTION-PATH MEASUREMENT GAP — sub-γ docs-only 2026-05-12: Phase 6 pass-record-integration sub-β (commit `999e9ff`) landed depth-attached production wire at `editor-shell::render_path::render_frame` (transient depth texture via `build_resource_map` over a `CompiledFrameGraph`-driven `TexturePool`; pipeline via `LitMeshPipeline::new_with_depth(.., Some(DepthStateKey { Depth24Plus, false, LessEqual }))`). The gfx-level Gate A harness above exercises NEITHER the sub-α `new_with_depth` API NOR the sub-β substrate consumption NOR the editor-shell production path. The 0.112 ms claim is VALID for the pre-depth gfx path and structurally SILENT about the post-depth production path. Post-depth Gate A re-measurement DEFERRED until either a synthetic depth-attached gfx-level harness lands OR a non-winit editor-shell perf harness lands OR a manual user report exists; see BASELINE.md §6.3 post-sub-β note.]**
