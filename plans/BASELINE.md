...
**Post-sub-β measurement gap (2026-05-12, docs-only sub-γ acknowledgment — Phase 6 pass-record-integration chapter close)**: Phase 6 pass-record-integration sub-β (commit `999e9ff`) landed transient depth-texture consumption at `editor-shell::render_path::render_frame` — production now constructs `LitMeshPipeline::new_with_depth(.., Some(DepthStateKey { Depth24Plus, depth_write_enabled: false, LessEqual }))` (sub-α additive API) and acquires a per-frame depth texture via `build_resource_map(...)` over a single-pass `CompiledFrameGraph`-driven `TexturePool` (dispatch-122 substrate-discipline path). The harness above (`crates/gfx/tests/gate_a_simple_scene_60fps.rs`) is gfx-level synthetic — it constructs `LitMeshPipeline::new(...)` (no depth-state) and calls `record_lit_mesh_pass(.., None)` (no depth attachment) — so it exercises NEITHER the sub-α depth-attached gfx primitives NOR the editor-shell production path. **The 0.112 ms claim above is VALID for the pre-depth gfx path and structurally SILENT about the post-depth production path.** Post-depth Gate A re-measurement DEFERRED until any of: (a) a synthetic depth-attached gfx-level harness lands (would mirror the existing setup but pass `Some(&depth_view)` + `Some(DepthStateKey { ... })`; still gfx-level not editor-shell-integrated; ~80-120 LoC additive); (b) a non-winit editor-shell perf harness lands (substantial — blocked on `EditorShell::render_frame` accepting a mock event loop); (c) the user runs the editor manually and reports observed perf. **No new perf number, no harness change, no architecture change in sub-γ.** Phase 6 pass-record-integration chapter closes at the docs-honest layer: API ready (sub-α), production substrate consumed (sub-β), perf/visual proof deferred until a real measurement path exists. Visual harness for the structural Z-fight-prevention claim made in sub-β (would graduate the regression-prevention to a user-visible-fix claim) is a parallel separate scope — not part of sub-γ.
